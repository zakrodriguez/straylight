#!/bin/bash
# scripts/checks/issueca.sh — extracted from validate.sh verbatim.
register_checks_issueca() {
# ─── ISSUECA (two-tier only) ────────────────────────────────────────────

if ! profile_has issueca; then
    skip_vm issueca "not in profile '$LAB_PROFILE_NAME'"
elif is_running issueca; then
        launch_check issueca run_windows_check issueca "$(cat <<'PS1'
...
# ── Web Enrollment (two-tier only — ISSUECA has certsrv) ──
# /certsrv has sslFlags=Ssl so it requires HTTPS. Use ICertificatePolicy via Add-Type
# instead of a script-block ServerCertificateValidationCallback — the latter raises
# 'no Runspace available' on PS 5.1 because the callback fires on a non-PS thread.
...
'@
    ...
    try {
        $r = Invoke-WebRequest -Uri https://localhost/certsrv/ -UseBasicParsing -TimeoutSec 10 -ErrorAction Stop
        Write-Output "PASS: Web Enrollment (certsrv) reachable (HTTP $($r.StatusCode))"
    } catch {
        $code = $null
        if ($_.Exception.Response) { $code = [int]$_.Exception.Response.StatusCode }
        if ($code -eq 401) {
            Write-Output "PASS: Web Enrollment (certsrv) reachable (HTTP 401, auth challenge)"
        } else {
            $msg = if ($code) { "HTTP $code" } else { $_.Exception.Message }
            Write-Output "FAIL: Web Enrollment (certsrv) not reachable ($msg)"
        }
    }
}
PS1
...
$(ps_check_cert_expiry)" "$TMPDIR_VAL/issueca"
else
        skip_vm issueca
fi
}
